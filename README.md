...
## Game Boy ROM sources

The Game Boy app downloads ROMs from manifest URLs. It works **without an SD card** by default — 
built-in sources like romsgames.net are always available. To customize sources, optionally create 
`/roms/sources.txt` on your microSD card.

Each source can be either:
- **Plain text manifest:** one ROM URL per line (optionally with display names)
- **HTML directory listing:** the app auto-parses links to `.gb` files

### Built-in sources (no setup required)

By default, the app provides:
- **ROMs Games (HTML)** — `https://www.romsgames.net/roms/gameboy/`

### Custom sources (optional — requires SD card)

To add your own sources, copy [`firmware/roms-sources.txt.example`](firmware/roms-sources.txt.example) 
to your microSD card as `/roms/sources.txt` and edit as needed.

Example `/roms/sources.txt`:

```
# Plain text manifests (one ROM URL per line inside the file)
https://example.com/roms/gameboy-manifest.txt | My ROMs

# HTML pages with .gb links (auto-parsed)
https://example.org/gb-collection/
```

On the device, open **Game Boy → [+ Download ROMs] → pick source → pick ROM**.
Downloaded files land in `/roms/` and appear in the picker on next launch.

### Manifest format

Plain text manifests support two formats per line:

```
# Just URLs (display name extracted from filename)
https://example.com/tetris.gb
https://example.com/super-mario-land.gb?format=zip

# With custom display names
Tetris | https://example.com/tetris.gb
Mario  | https://example.com/mario.gb
```

HTML pages are parsed automatically — all `.gb` links are extracted, relative
URLs resolved, and duplicates filtered.
